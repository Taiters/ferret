...
import { parseFile, chunkPlainText } from "./parser.js";
import { parseMarkdown } from "./markdown.js";
import { parseGitHistory } from "./git.js";
...
// ── Main ──────────────────────────────────────────────────────────────────────
export async function indexProject(projectPath, { gitLimit = 100, verbose = false } = {}) {
  ...
  const allChunks = [];
  const callGraph = new Map(); // fnName → { calls[], file }
  ...
  console.log("\n  🕸  Building call graph...");
  const calledBy = new Map(); // fnName → Set of callers
  ...
  for (const [name, { calls, file }] of callGraph) {
